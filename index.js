...
function startDB() {
  inquirer
    .prompt({
      ...
      name: "start",
      message:
        "Hello! Welcome to the Employee Database. Would you like to begin?",
    })
    .then((res) => {
      if (res.start) {
        navDB();
...
function viewDepts() {
  ...
  db.query(criteria, function (err, res) {
    ...
    console.table(res);
    console.log(
      "Please press an arrow key to return to the menu.\n\n\n\n\n\n\n"
    );
  });
  navDB();
}
function viewRoles() {
  let criteria =
    "SELECT role.id AS ID, role.title AS Title, role.salary AS Salary, department.dept_name AS Department FROM role JOIN department ON role.dept_id = department.id";
  db.query(criteria, function (err, res) {
    if (err) throw err;
    console.log("\n");
    console.table(res);
...
function addRole() {
  inquirer
    ...
    .then((res) => {
      ...
      let q1 =
        "SELECT department.id AS ID, department.dept_name AS Department FROM department WHERE department.dept_name = ?";
